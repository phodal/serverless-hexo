...
let create = (event, context, callback) => {
  ...
  let blogZipFile = '/tmp/blog.zip';
  let repoUnzipPath = "/tmp/serverless-hexo-blog-static-files-master";
  let bucketName = 'static.wdsm.io';

  request.get(`${repoAddress}/archive/master.zip`)
    .pipe(fs.createWriteStream(blogZipFile))
    .on('close', (repo) => {fs.createReadStream(blogZipFile)
        .pipe(unzip.Extract({path: '/tmp/'}))
        .on('close', function () {
          console.log(repo);
          console.info(`Download ${repoAddress}`);
